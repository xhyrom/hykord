...
waitForSync(
  (m) => m.Tags && Filters.byCode('errorSeparator')(m),
  (m) => (Forms.FormTitle = m),
);
waitForSync(
  (m) => m.Tags && Filters.byCode('titleClassName', 'sectionTitle')(m),
  (m) => (Forms.FormSection = m),
);
waitForSync(
  (m) => m.Types?.INPUT_PLACEHOLDER,
  (m) => (Forms.FormText = m),
);
waitForSync(
  Filters.byCode('().divider', 'style'),
...
waitForSync(
  Filters.byCode('input', 'createElement', 'checkbox'),
  (m) => (Checkbox = m),
);
...
waitForSync(
  (m) => {
    ...
    return (
      s.length < 1500 &&
      s.includes('data-text-variant') &&
      s.includes('always-white')
    );
  },
  (m) => (Text = m),
);
...
waitForSync(Filters.byCode('onCloseRequest:null!='), (m) => {
  Modals.openModal = (Component, modalProps, key) => {
    ...
    m((props) => <Component {...props} {...modalProps} />, { modalKey: key });
...
waitForSync(
  ...
  (m) => (Modals.closeModal = m),
);
waitForSync(
  Filters.byCode('headerIdIsManaged:'),
  (m) => (Modals.ModalRoot = m),
);
waitForSync(
  Filters.byCode(
    'children',
  ...
  (m) => (Modals.ModalHeader = m),
);
waitForSync(
  Filters.byCode('scrollerRef', 'content', 'className', 'children'),
  (m) => (Modals.ModalContent = m),
);
waitForSync(
  Filters.byCode(
    'HORIZONTAL_REVERSE',
  ...
  (m) => (Modals.ModalFooter = m),
);
waitForSync(
  Filters.byCode('closeWithCircleBackground', 'hideOnFullscreen'),
  (m) => (Modals.ModalCloseButton = m),
);
