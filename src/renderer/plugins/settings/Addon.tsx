...
export default ErrorBoundary.wrap((props: Props) => {
    ...
    const allAddons: Addon[] = input === ''
        ? [...addons]
        : addons.filter(addon => addon.name.toLowerCase().includes(input.toLowerCase()));

    return (
        <Forms.FormSection tag='h1' title='Plugins'>
        <Forms.FormTitle>{title}</Forms.FormTitle>
        <TextInput value={input} placeholder={placeholder} onChange={setInput} style={{ marginBottom: 24 }} />
            {allAddons.map((addon) => (
                <AddonCard
                    name={addon.name}
                    type={type}
