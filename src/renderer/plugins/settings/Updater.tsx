      ...
      body={
        <>
          {Updater.changes.map((commit) => (
            <Forms.FormText>
              <Link href={`${details.repositoryUrl}/commit/${commit.hash}`}>
                {commit.hash}
              </Link>{' '}
...
export default ErrorBoundary.wrap(() => {
  ...
  React.useEffect(() => {
    (async () => {
      const repo = await HykordNative.getManagers()
        .getUpdater()
        .getRepository();
      setDetails({
        repositoryName: repo.name,
        repositoryUrl: repo.url,
        hash: await HykordNative.getManagers()
          .getUpdater()
          .getLatestCommitHash(),
      });
    })();
  ...
  return (
    <Forms.FormSection tag="h1" title="Updater">
      <Forms.FormText>
        <Link href={details.repositoryUrl}>{details.repositoryName}</Link> (
      ...
      {Updater.isOutdated() ? (
        ...
        <Forms.FormText type={Forms.FormText.Types.LABEL_BOLD}>
          Hykord is up to date!
        </Forms.FormText>
      )}
      <br />
      <Flex>
