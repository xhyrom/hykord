...
import { Alerts, Card, Checkbox, ErrorBoundary, Flex, Forms, Inputs, Tooltip } from '@hykord/components';
...
const Settings = (props: { settings: PluginSetting[], addon: Plugin | Theme }) => {
  return <div style={
    {
      display: 'inline',
      textAlign: 'left',
    ...
    {props.settings.map((setting) => (
      <Inputs.Switch
        value={HykordNative.getManagers()
          .getSettings()
          .getSync(`plugins.${props.addon.$cleanName}.${setting.name}`, setting.defaultValue)}
          ...
          >
            ...
            <div className='hykord-card-right'>
              <div className='hykord-card-buttons'>
                { (addon as Plugin).settings && <Tooltip position={Tooltip.Positions.RIGHT} text='Open Settings' align='center'>
                  {(tooltipProps: any) => (
                    <svg
                      {...tooltipProps}
                      onClick={() => {
                          Alerts.show({
                            ...
                            body: <Settings settings={(addon as Plugin)!.settings!} addon={addon} />,
                            confirmText: ''
                          })
                        }
                      }
