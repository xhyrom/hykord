...
// https://github.com/BetterDiscord/BetterDiscord/blob/main/renderer/src/modules/webpackmodules.js
 ...
 export default class WebpackModules {
     static find(filter, first = true) {return WebpackModules.getModule(filter, {first});}
     static findAll(filter) {return WebpackModules.getModule(filter, {first: false});}
     static findByUniqueProperties(props, first = true) {return first ? WebpackModules.getByProps(...props) : WebpackModules.getAllByProps(...props);}
     static findByDisplayName(name) {return WebpackModules.getByDisplayName(name);}
     ...
     static getModule(filter, options = {}) {
         ...
         const wrappedFilter = (exports, module, moduleId) => {
             try {
                 ...
                 if (!hasThrown.has(filter)) return;//Logger.warn("WebpackModules~getModule", "Module filter threw an exception.", filter, err);
             ...
             for (let q = 0; q < queries.length; q++) {
                 ...
                 const wrappedFilter = (ex, mod, moduleId) => {
                     try {
                         ...
                         if (!hasThrown.has(filter)) return;//Logger.warn("WebpackModules~getBulk", "Module filter threw an exception.", filter, err);
                         hasThrown.add(filter);
                         return false;
                     }
                 };
      ...
      * @param {Function} filter A function to use to filter modules
      */
     static getModules(filter) {return WebpackModules.getModule(filter, {first: false});}
     ...
     static getByDisplayName(name) {
         return WebpackModules.getModule(Filters.byDisplayName(name));
     }
     ...
     static getByRegex(regex, first = true) {
         return WebpackModules.getModule(Filters.byRegex(regex), {first});
     }
      ...
      */
     static get require() {
         if (WebpackModules._require) return WebpackModules._require;
         ...
         else if (typeof(window[WebpackModules.chunkName]) !== "undefined") {
             window[WebpackModules.chunkName].push([[id], 
                 {},
         ...
         return WebpackModules._require = __discord_webpack_require__;
     }
     ...
     static getAllModules() {
         return WebpackModules.require.c;
     }
     ...
     static initialize() {
         WebpackModules.handlePush = WebpackModules.handlePush.bind(this);
         WebpackModules.listeners = new Set();
         
         WebpackModules.__ORIGINAL_PUSH__ = window[WebpackModules.chunkName].push;
         Object.defineProperty(window[WebpackModules.chunkName], "push", {
             configurable: true,
             get: () => WebpackModules.handlePush,
             set: (newPush) => {
                 WebpackModules.__ORIGINAL_PUSH__ = newPush;
 
                 Object.defineProperty(window[WebpackModules.chunkName], "push", {
                     value: WebpackModules.handlePush,
                     configurable: true,
                     writable: true
                 });
             }
      ...
      */
      static addListener(listener) {
         WebpackModules.listeners.add(listener);
         return WebpackModules.removeListener.bind(this, listener);
     }
      ...
      * @returns {boolean}
      */
     static removeListener(listener) {return WebpackModules.listeners.delete(listener);}
     ...
     static handlePush(chunk) {
         ...
         return Reflect.apply(WebpackModules.__ORIGINAL_PUSH__, window[WebpackModules.chunkName], [chunk]);
     }
 }
